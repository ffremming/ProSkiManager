...
export default function RaceSetupPage() {
  const { team, athletes, equipment, racePrep, setRacePrep } = useGameStore((s) => {
  ...
  const [raceId, setRaceId] = useState(raceCourses.vasaloppet.id);
  ...
  const conditions = raceConditions[raceId];

  const course = raceCourses[raceId];
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-6">
        ...
        <section className="grid gap-3 md:grid-cols-3">
          <div className="rounded-xl border border-white/10 bg-white/5 p-3">
            ...
            >
              {Object.values(raceCourses).map((r) => (
